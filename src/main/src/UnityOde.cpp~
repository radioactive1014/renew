...
static void nearCallback (void *data, dGeomID o1, dGeomID o2)
{
	...
	//printf("body %f\ n",dGeomGetBody(o1));
...
bool initOde(int numThreads)
{
	...
	for (int i=0; i<numThreads; i++)
	{
		...
		dWorldSetGravity (contexts[i].world,0,0,-9.81f);
...
void EXPORT_API stepOde(int com)
{
	...
	if (com==1)
         {
	     int cp =0 ;  // doing nothing
	}


	else {

		dsDrawBox( dBodyGetPosition(c.bodies[1]),dBodyGetRotation(c.bodies[1]),side2);
		dsDrawCapsule( dBodyGetPosition(c.bodies[3]),dBodyGetRotation(c.bodies[3]),0.2-0.05,0.05f);
	...
	//dsDrawCapsule( dBodyGetPosition(c.bodies[13]),dBodyGetRotation(c.bodies[10]),0.20,0.1f);
	
	
	//dsDrawSphere( dBodyGetPosition(c.bodies[22]),dBodyGetRotation(c.bodies[22]),0.05f);
	}
