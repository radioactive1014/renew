	...
	*/
...

	{
...

		{
			...
			float cost=squared((-0.08-rel)*12.0f)+squared((0.00-rel_second)*12.0f);//+squared(angle*5)+squared(angle_second*5);//+ squared(angle*30.0f) ;


			if (-0.07<rel_second && rel_second <0.07 && -0.07<rel && rel<0.07 )
{
...
int main(int argc, char **argv)
{
	...
	*/
	...
	obs.body = odeBodyCreate();
	obs.geom = odeCreateBox(0.05,0.05,0.02); 
	odeMassSetBoxTotal(obs.body,0.09,0.05,0.05,0.02 ); 
	odeBodySetPosition(obs.body,0.00,0.0,h_floor_table+h_base+h_sphere+h_support+stage_dim[2]+obs.radius);
	//odeGeomSetBody(obs.geom,obs.body);
	printf("capsule obstacle body id %f, geom id %f \n", obs.body, obs.geom);
