...
void robot(int com)
{
...

	{
...

		{
			...
			float cost=squared((0.07-pos[0])*25.0f)+squared((0.07-pos[1])*25.0f);//+ squared(angle*30.0f) ;
	...
	*/
	...
	//printf("rel  %f \n", rel1);
	printf("FINAL Posx %1.3f, posy %1.3f, posz = %f  angle %1.3f, cost=%1.3f, control %f \n",pos1[0],pos1[1],pos1[2],angle1*180/3.1416,cost,control[0]);
...
int main(int argc, char **argv)
{
	...
	*/
	...
	odeBodySetPosition(ball.body,-0.05,-0.12,h_floor_table+h_base+h_sphere+h_support+stage_dim[2]);
	...
	odeMassSetBoxTotal(obs.body, 0.05,0.05,0.05,0.02); 
