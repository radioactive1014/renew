...
float minControl=-5.5;	//lower sampling bound
float maxControl=5.5;	//upper sampling bound
...
void robot(int com)
{
...

	{
...

		{
			...
			float cost=squared((pos[1])*50.0f);//+ squared(angle*30.0f) ;
...
//creating stage
	...
	odeBodySetPosition(stage.body,0,0.0,h_base+h_sphere+h_support+stage_dim[2]/2);
	...
	odeBodySetPosition(support.body,0.0f,0.0f,h_base+h_sphere+(h_support/2)-stage_dim[2]/2);
	...
	odeBodySetPosition(LinkBall.body,0,0,h_base-support.radius);
	...
	odeBodySetPosition(mainLink.body,(mainLink.length+mainLink.radius),0, h_base-mainLink.radius);
	...
	odeBodySetPosition(ball.body,0.0,-0.05,h_base+h_sphere+h_support+stage_dim[2]);
