...
float starting_pos = -0.1 ; 
float last_pos;
float current_pos = starting_pos;
...
void robot(int com)
{
	...
	const dReal *vel, *pos_vel;

	pos_vel = odeBodyGetPosition(ball.body);

	current_pos = pos_vel[1] ;	
	float vel_manual = (current_pos- last_pos)/(1.0f/24.0f); 

	
	vel =odeBodyGetLinearVel(ball.body);
	printf("vel of the body %f, vel manual %f \n", vel[1], vel_manual);

	//odeBodySetLinearVel(ball.body, vel[0], vel[1], vel[2]);


	float angle_f=odeJointGetHingeAngle(mainLink.joint);
	printf("angle before %f \n", angle_f);

	float time = 0.041666667 ;
	dReal MaxForce = dInfinity;
	dReal Gain = 5 ;
	dReal DesiredPosition = angle_f-0.1;
	dReal Error = angle_f - DesiredPosition;
	printf("error %f \n", Error);
	printf("time %f \n", time);

	dReal DesiredVelocity = -Error*24;

	//float target = 0.1 ; 
	//float time = 1/24 ;
	//float desiredVelocity = (angle_f -target) * time ; 
...
//}	
...

	{
...

		{
			...
			float cost=squared((pos[1])*30.0f);//+ squared(angle*30.0f) ;
	...
	*/
	...
	odeJointSetHingeParam(mainLink.joint,dParamFMax,dInfinity);
	odeJointSetHingeParam(mainLink.joint,dParamVel,0.2);
	...
	pos1 = odeBodyGetPosition(ball.body);
	angle1=odeJointGetHingeAngle(mainLink.joint);
...
void start()
{
	static float xyz[3] = {0.0,-1.6,2.0};
...
int main(int argc, char **argv)
{
	...
	odeBodySetPosition(ball.body,0.00,starting_pos,h_floor_table +h_base+h_sphere+h_support+stage_dim[2]+ball.radius/2);
